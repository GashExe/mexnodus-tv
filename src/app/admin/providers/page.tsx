...
export default async function ProvidersPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Riesgo del embed: SOLO analítica. En web los iframes van SIN sandbox. */}
      <div>
        <h2 className="text-lg font-semibold tracking-tight">Riesgo de embeds (analítica)</h2>
        <p className="mt-1 text-sm text-ink-3">
          En la versión <strong>web</strong> los proveedores de embed se renderizan{" "}
          <strong>sin el atributo <code>sandbox</code></strong> a propósito (múltiples proveedores lo
          detectan y rechazan la reproducción). Los iframes usan{" "}
          <code className="text-ink-2">allow=&quot;autoplay; fullscreen; picture-in-picture; encrypted-media&quot;</code>{" "}
          y <code className="text-ink-2">referrerPolicy=&quot;strict-origin-when-cross-origin&quot;</code>.
          Estos riesgos son informativos: alimentan la futura política de bloqueo de popups en las
          apps nativas (Electron/Android/iOS).
        </p>

        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          {providers.filter(isEmbedProvider).map((p) => {
            const sec = readProviderSecurity(p.public_config);
            const needsNative = sec.popup_risk === "high" || sec.redirect_risk === "high";
            return (
              <div key={p.id} className="rounded-card border border-line bg-surface p-4">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Chip tone={needsNative ? "gold" : "accent"}>
                    {needsNative ? "mitigar en nativo" : "riesgo bajo"}
                  </Chip>
                </div>
                ...
                <div className="mt-2 font-mono text-[11px] text-ink-3">
                  última evaluación: {fmtDate(sec.last_security_test_at)}
                </div>
